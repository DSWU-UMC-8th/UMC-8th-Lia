...
//   // 1. 상태 변수: throttledValue : 최종적으로 throttling이 적용된 값
//   // 초기값을 전달받은 value
//   const [throttledValue, setThrottledValue] = useState<T>(value);

//   // 2. Ref lastExecuted: 마지막으로 실행된 시간을 기록하는 변수
//   // useRef를 사용하면 컴포넌트가 리렌더링 되어도 값이 유지되고
//   // 변경되어도 리렌더링을 트리거하지 않음
//   const lastExecuted = useRef<number>(Date.now());

//   // 3. useEffect: value, delay가 변경될 때 아래 로직 실행
//   useEffect(() => {
//     // 현재 시각과 lastExecuted.current에 저장된 마지막 시각 + dalay를 비교
//     // 충분한 시간이 지나면 바로 업데이트
//     if (Date.now() >= lastExecuted.current + delay) {
//       // 현재 시간이 지난 경우
//       // 현재 시각으로 lastExecuted를 업데이트
